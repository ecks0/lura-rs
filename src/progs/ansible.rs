...
use {
  ...
  thiserror,
  crate::run::{Error as RunError, Runner},
};
...
impl Ansible {
  ...
  pub fn playbook(&self, runner: Runner, playbook_path: &str) -> Result<()> {
    let mut args = self.get_args();
    args.insert(0, playbook_path.to_owned());
    let code = &runner.run("ansible-playbook", args.iter())?.code();
    if 0.eq(code) {
      Ok(())
    } else {
  ...
  pub fn module<I>(&self, runner: Runner, module: &str, module_args: &str) -> Result<()>
  {
    let mut args = self.get_args();
    ...
    let code = &runner.run("ansible", args.iter())?.code();
    if 0.eq(code) {
      Ok(())
    } else {
      Err(Error::from(RunError::UnexpectedExitCode(*code)))
